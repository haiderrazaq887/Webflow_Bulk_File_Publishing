...
async function publishToWebflow(data) {
    try {
        ...
        // ✅ STEP 2: CORRECT publish endpoint (NO fieldData here)
        await axios.post(
            `https://api.webflow.com/v2/collections/${process.env.COLLECTION_ID}/items/publish`,
            {
                itemIds: [itemId]   // ONLY this
            },
            {
                headers: {
                    Authorization: `Bearer ${process.env.WEBFLOW_API_KEY}`,
...
app.post('/upload', upload.array('files'), async (req, res) => {
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        try {
            ...
            paragraphs.shift(); // remove first paragraph

            // Rebuild remaining content
    ...
    res.send('All files processed and published!');
});
